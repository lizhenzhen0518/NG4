...
import { Component, OnDestroy, OnInit, forwardRef, ChangeDetectionStrategy } from '@angular/core';
import { IndentityType, Identity } from '../../domain';
import { Subject } from  'rxjs/Subject';
import { Subscription } from 'rxjs/Subscription';
import { Observable } from 'rxjs/Observable';
import { isValidAddr, extractInfo } from '../../util/identity.util';
...
export class  IdentityInputComponent implements ControlValueAccessor, OnInit, OnDestroy {
  ...
  identityTypes: {value: IndentityType, label: string}[] = [
    {value: IndentityType.IdCard, label: '身份证'},
    {value: IndentityType.Insurance, label: '医保'},
    {value: IndentityType.Passport, label: '护照'},
    {value: IndentityType.Military, label: '军官证'},
    {value: IndentityType.Other, label: '其他'}
  ];
  identity: Identity = {identityType: null, identityNo: null};
  private _idType = new Subject<IndentityType>();
  ...
  validate(c: FormControl): {[key: string]: any} {
    ...
    switch (c.value.identityType) {
      case IndentityType.IdCard: {
        return this.validateIdNumber(c);
      }
      case IndentityType.Passport: {
        return this.validatePassport(c);
      }
      case IndentityType.Military: {
        return this.validateMilitary(c);
      }
      case IndentityType.Insurance:
      default: {
        return null;
      }
    }
  ...
  private get idType(): Observable<IndentityType> {
